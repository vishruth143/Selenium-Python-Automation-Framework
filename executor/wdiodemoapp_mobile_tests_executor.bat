...
REM Default execution context for WDIO Demo App mobile tests.
set REGION=QA

REM Run WDIO Demo App mobile tests with verbose logs, parallel workers, HTML report, and Allure raw results.
pytest -vvv -m "wdio" --log-cli-level=INFO --html=output/reports/report.html --alluredir=output/allure-results --self-contained-html --capture=tee-sys --durations=10 tests
